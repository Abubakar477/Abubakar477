...
These are files in your own repo, so they render as long as GitHub renders.
    ...
    python scripts/cards.py --user Abubakar477 --out assets
...
def rest(path: str, token: str | None):
    ...
    try:
        ...
        sys.exit(f"GitHub API error on {path}: {e}")


def icon(path_d: str, x: float, y: float, size: float, color: str) -> str:
    return (
        f'<svg x="{x}" y="{y}" width="{size}" height="{size}" viewBox="0 0 16 16" fill="{color}">'
        f'<path d="{path_d}"/></svg>'
    )
...
def fetch_contributions(user: str, token: str | None):
    ...
    if "errors" in data or "data" not in data or not data["data"]["user"]:
        print(f"GraphQL error or user not found: {data.get('errors')}", file=sys.stderr)
        return None
...
def render_repo(repo: dict, theme: str) -> str:
    ...
    for path_d, count in ((ICON_STAR, repo.get("stars", 0)),
                          (ICON_FORK, repo.get("forks", 0))):
        out.append(icon(path_d, x, fy - 11, 12, c["muted"]))
        out.append(
...
def main(argv=None):
    ...
    token = os.environ.get("GITHUB_TOKEN") or os.environ.get("GH_TOKEN") or os.environ.get("METRICS_TOKEN")
    ...
    for entry in wanted:
        ...
        print(f"wrote card-{src['name']}-*.svg ({card['stars']}★ {card['forks']}⑂ {card['language']})")
